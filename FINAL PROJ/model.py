...
class Model(GANModelDesc):
    ...
    def generator(self, img):
        with argscope([Conv2D, Deconv2D],
            ...
            subDepth = 3
            conv0 = Conv2D('conv0', img, NF_G, nl=tf.nn.relu)
            conv1 = Conv2D('conv1', conv0, NF_G * 2)
            layer1 = res_group(conv1, 'layer1', subDepth, NF_G*2)
            conv2 = Conv2D('conv2', layer1, NF_G * 4)
            layer2 = res_group(conv2, 'layer2', subDepth, NF_G*4)
            conv3 = Conv2D('conv3', layer2, NF_G * 8)
            l = res_group(conv3, 'layer3', subDepth, NF_G*8)
            deconv0 = Deconv2D('deconv0', l, NF_G * 4)
            up1 = tf.concat([deconv0, layer2], axis=1)
            b_layer_2 = res_group(up1, 'blayer2', subDepth, NF_G * 4)
            deconv1 = Deconv2D('deconv1', b_layer_2, NF_G * 2)
            up2 = tf.concat([deconv1, layer1], axis=1)
            b_layer_1 = res_group(up2, 'blayer1', subDepth, NF_G * 2)
            deconv2 = Deconv2D('deconv2', b_layer_1, NF_G * 1)
            deconv3 = Deconv2D('deconv3', deconv2, 3, nl=tf.sigmoid)
        return deconv3

    def discriminator(self, img):
        with argscope(Conv2D, nl=INLReLU, kernel_shape=4, stride=2):
            l = Conv2D('conv0', img, NF_D*2, nl=tf.nn.relu)
            relu1 = Conv2D('conv1', l, NF_D * 4)
            relu2 = Conv2D('conv2', relu1, NF_D * 8)

            relu3 = Conv2D('convf', relu2, NF_D*8, kernel_shape=3, stride=1)
            atrous = tf.contrib.layers.conv2d(relu3, NF_D*8, kernel_size=3,
                    data_format='NCHW', rate=2,
                    activation_fn=INLReLU, biases_initializer=None)
            atrous2 = tf.contrib.layers.conv2d(atrous, NF_D*8, kernel_size=3,
                    data_format='NCHW', rate=4,
                    activation_fn=INLReLU, biases_initializer=None)
            atrous3 = tf.contrib.layers.conv2d(atrous2, NF_D*8, kernel_size=3,
                    data_format='NCHW', rate=8,
                    activation_fn=INLReLU, biases_initializer=None)
            merge = tf.concat([relu3, atrous3], axis=1)
            clean = Conv2D('mConv', merge, NF_D*8, kernel_shape=3, stride=1)
            lsgan = Conv2D('lsconv', clean, 1, stride=1, nl=tf.identity,
                    use_bias=False)
        return lsgan, [atrous3], [atrous, atrous2, atrous3]
    ...
    def get_style_loss(self, feats_real, feats_fake):
        losses = []
        for real, fake in zip(feats_real, feats_fake): # NCHW
            G_real = self.gram_matrix(real)
            G_fake = self.gram_matrix(fake)
            loss = tf.reduce_mean( tf.squared_difference(G_real, G_fake) )
            losses.append(loss)
        ret = tf.reduce_mean(losses, name='style_loss')
        add_moving_summary(ret)
        return ret
    ...
    def build_graph(self, A, B):
        ...
        with argscope([Conv2D, Deconv2D, FullyConnected],
            ...
            with tf.variable_scope('discrim'):
                with tf.variable_scope('A'):
                    A_dis_real, A_feats_real_FM, A_feats_real_SL = self.discriminator(A)
                with tf.variable_scope('A', reuse=True):
                    A_dis_fake, A_feats_fake_FM, A_feats_fake_SL = self.discriminator(BA)

                with tf.variable_scope('B'):
                    B_dis_real, B_feats_real_FM, B_feats_real_SL = self.discriminator(B)
                with tf.variable_scope('B', reuse=True):
                    B_dis_fake, B_feats_fake_FM, B_feats_fake_SL = self.discriminator(AB)
        ...
        with tf.name_scope('LossA'):
            ...
            fm_loss_A = self.get_feature_match_loss(A_feats_real_FM, B_feats_fake_FM)
            style_loss_A = self.get_style_loss(A_feats_real_SL, A_feats_fake_SL)
        ...
        with tf.name_scope('LossB'):
            ...
            fm_loss_B = self.get_feature_match_loss(B_feats_real_FM, A_feats_fake_FM)
            style_loss_B = self.get_style_loss(B_feats_real_SL, B_feats_fake_SL)
        ...
        rate = tf.constant(0.5, np.float32, name='static_rate')

        g_loss = tf.add_n([
            (self.loss_normalize(G_loss_A + G_loss_B, loss_update) * 0.85 +
            (self.loss_normalize(fm_loss_A + fm_loss_B, loss_update) * 0.2 + 
            self.loss_normalize(style_loss_A + style_loss_B, loss_update) * 0.8) * 0.15) * (1 - rate),
